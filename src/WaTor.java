...
public class WaTor { 
    ...
    public static void showFishAndSharks(int chronon, int[][] fish, int[][] sharks) {
        ...
        for (int y = 0; y < fish.length; y++){
            for (int x = 0; x < fish[0].length; x++){
                if (fish[y][x] >= 0){
                    System.out.print( Config.FISH_MARK + " ");
                    numFish += 1;

                } else if(sharks[y][x] >= 0){
    ...
    public static int placeFish(int[][] fish, int startingFish, int fishBreed, Random randGen) {
        ...
        for (int i = 0; i < startingFish; i++){
            ...
            for (int p = 1; p < Config.MAX_PLACE_ATTEMPTS; p++){

                int y = randGen.nextInt(fish.length);
                int x = randGen.nextInt(fish[0].length);

                if (fish[y][x] == Config.EMPTY){
                    fish[y][x] = randGen.nextInt(fishBreed + 1);
                    numFishPlaced += 1;
                    placed = true;
                    break;
                }
    ...
    public static int placeSharks(int[][] fish, int[][] sharks, int startingSharks,
        ...
        for (int i = 0; i < startingSharks; i++){
            ...
            for (int p = 1; p < Config.MAX_PLACE_ATTEMPTS; p++){
                ...
                int y = randGen.nextInt(sharks.length);
                int x = randGen.nextInt(sharks[0].length);

                if ((sharks[y][x] == Config.EMPTY) && (fish[y][x] == Config.EMPTY)){
                    sharks[y][x] = randGen.nextInt(sharksBreed + 1);
                    numSharksPlaced += 1;
                    placed = true;
                    break;
                }
    ...
    public static ArrayList<int[]> unoccupiedPositions(int[][] fish, int[][] sharks, 
        ...
        if (row == 0){
            if ((fish[fish.length][col] == Config.EMPTY) && (sharks[sharks.length][col] == Config.EMPTY)){
                int[] c = {fish.length, col};
                unoccupied.add(c);
            }
        } else{
        ...
        if (col == 0){
            if ((fish[row][fish[0].length] == Config.EMPTY) && (sharks[row][sharks[0].length] == Config.EMPTY)){
                int[] c = {row, fish[0].length};
                unoccupied.add(c);
            }
        } else{
    ...
    public static int[] chooseMove(ArrayList<int[]> neighbors, Random randGen) {
        ...
        try{
            if(neighbors == null){
                throw new Exception("null exception for neighbors");
            }
            if (neighbors.size() == 1){
                ...
                if (randGen == null){
                    throw new Exception("null exception for randGen");
                }
                int m = randGen.nextInt(neighbors.size());
                return neighbors.get(m);
    ...
    public static int fishSwimAndBreed(int[][] fish, int[][] sharks, boolean[][] fishMove,
        ...
        try{
            if ((fish == null) || (sharks == null) || (fishMove == null)){
            ...
            if (fishBreed < 0){
                throw new Exception("fishBreed < 0");

            }

            if (randGen == null){
                throw new Exception("randGen is null");

            }
            for (int row = 0; row < fish.length; row++){
                for (int col = 0; col < fish[0].length; col++){
                    if (fishMove[row][col] == false){
                        ArrayList<int[]> unOc = unoccupiedPositions(fish, sharks, row, col);

                        int[] coord = chooseMove(unOc, randGen);

                        if ( coord == null){ //if there is no unoccupied spaces
                            aFishStays(fish, fishMove, row, col);

                        }else{
                            if(fish[row][col] == fishBreed){ //if the fish here is of breeding age, Swim and Breed
                                aFishMovesAndBreeds(fish, fishMove, row, col, coord[0], coord[1]);

                            }else{
                                aFishMoves(fish, fishMove, row, col, coord[0], coord[1]);
            ...
            System.err.println(excpt.getMessage());
            if (excpt.getMessage().equals("Invalid Parameter(s)"))
                return -1;

            if (excpt.getMessage().equals("fishBreed < 0"))
                return -2;

            if (excpt.getMessage().equals("randGen is null"))
                return -3;
    ...
    public static int sharksHuntAndBreed(int[][] fish, int[][] sharks, boolean[][] fishMove,
        ...
        for (int row = 0; row < sharks.length; row++){
            for (int col = 0; col < sharks[0].length; col++){
                if (sharksMove[row][col] == false){

                    try {


                        if(( fish == null) || (sharks == null) || (fishMove == null) || (sharksMove == null ) || (starve == null)){
                            throw new Exception ("shark, fish, fishMove, sharksMove, and/or starve are null");

                        }

                        if ((sharksBreed < 0) || (sharksStarve < 0)){
                            throw new Exception ("SharksBreed annd/or sharksStarve are less than 0");
                        ...
                        if(sharks[row][col] == sharksStarve ){
                            sharkStarves(sharks, sharksMove, starve, row, col);

                        } else {
                            ArrayList<int[]> fishPos = fishPositions(fish, row, col);

                            if(fishPos.size() == 0){ //no neighboring fish
                                ArrayList<int[]> unOc = unoccupiedPositions(fish, sharks, row, col);
                                ...
                                if ( coord == null){ //if there is no unoccupied spaces
                                    ...
                                    if(sharks[row][col] == sharksBreed){ //if the sharks here is of breeding age, move and Breed
                                        sharkMovesAndBreeds(sharks, sharksMove, starve, row, col, coord[0], coord[1]);

                                    }else{
                                        sharkMoves(sharks, sharksMove, starve, row, col, coord[0], coord[1]);

                                    }                      
                                }

                            } else {
                                int[] coord = chooseMove(fishPos, randGen);
                                ...
                                if ( coord == null){ //if there is no unoccupied spaces
                                    ...
                                    if(sharks[row][col] == sharksBreed){ //if the sharks here is of breeding age, move and Breed
                                        sharkEatsFishAndBreeds(sharks, sharksMove, starve, fish, fishMove, row, col, coord[0], coord[1]);

                                    }else{
                                        sharkEatsFish(sharks, sharksMove, starve, fish, fishMove, row, col, coord[0], coord[1]);
                        ...
                        if (exc.getMessage().equals("shark, fish, fishMove, sharksMove, and/or starve are null"))
                            return -1;

                        if (exc.getMessage().equals("SharksBreed annd/or sharksStarve are less than 0"))
                            return -2;

                        if (exc.getMessage().equals("randGen is null"))
                            return -3;
